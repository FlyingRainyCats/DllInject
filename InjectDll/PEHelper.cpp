...
{
    hProcess_ = OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, FALSE, pid);
    auto IsWow64Process2fn = (tIsWowProcess2)GetProcAddress(GetModuleHandle(TEXT("kernel32.dll")), "IsWow64Process2");
    if (IsWow64Process2fn != nullptr) {
        ...
        if (IsWow64Process(hProcess_, &isWow64)) {
            ...
            machine_type_ = isProcessX86 ? IMAGE_FILE_MACHINE_I386 : IMAGE_FILE_MACHINE_AMD64;
        }
    }
}
...
{
    size_t unused{ 0 };
    void* ptr = VirtualAllocEx(hProcess_, nullptr, len, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READ);
    if (ptr) WriteProcessMemory(hProcess_, ptr, code, len, &unused);
    return ptr;
}
